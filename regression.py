...
y_pred_proba = lr.predict_proba(X_test_features)
...
print(f"⏱ Training Time (LR): {train_time:.2f} sec")
print(f"⏱ Testing Time (LR): {test_time:.2f} sec")
...
lr_prediction = lr.predict(img_features)
lr_pred_proba = lr.predict_proba(img_features)
pred_time = time.time() - start_pred_time

class_labels = list(train_data.class_indices.keys())
for i, prob in enumerate(lr_pred_proba[0]):
    print(f"Class '{class_labels[i]}': Probability = {prob:.4f}")

predicted_label = class_labels[lr_prediction[0]]
