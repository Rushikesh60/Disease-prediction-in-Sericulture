...
# -------------------------
# Step 4: CNN Feature Extractor (no softmax layer)
# -------------------------
...
cnn_model = Sequential([
    ...
    Dropout(0.5)
    #no soft max for feature extraction as it classifies using probability to class
])

#cnn_model.compile(optimizer=Adam(), loss="categorical_crossentropy", metrics=["accuracy"])
#not necessary while cnn used for feature extraction only but needed when we model.fit
# -------------------------
...
knn = KNeighborsClassifier(n_neighbors=5, metric="euclidean")  # <-- KNN instead of SVM
...
print(f"⏱ Training Time (KNN): {train_time:.2f} sec")
print(f"⏱ Testing Time (KNN): {test_time:.2f} sec")
...
knn_prediction = knn.predict(img_features)
knn_pred_proba = knn.predict_proba(img_features)
pred_time = time.time() - start_pred_time

class_labels = list(train_data.class_indices.keys())
for i, prob in enumerate(knn_pred_proba[0]):
    print(f"Class '{class_labels[i]}': Probability = {prob:.4f}")

predicted_label = class_labels[knn_prediction[0]]
