...
y_pred_proba = nb.predict_proba(X_test_features)
...
print(f"⏱ Training Time (NB): {train_time:.2f} sec")
print(f"⏱ Testing Time (NB): {test_time:.2f} sec")
...
nb_prediction = nb.predict(img_features)
nb_pred_proba = nb.predict_proba(img_features)
pred_time = time.time() - start_pred_time

class_labels = list(train_data.class_indices.keys())
for i, prob in enumerate(nb_pred_proba[0]):
    print(f"Class '{class_labels[i]}': Probability = {prob:.4f}")

predicted_label = class_labels[nb_prediction[0]]
